...
-- triangle = [[1],[2,3],[4,5,206],[200,8,9,10]]

solution = best_path (head . head $ triangle,0) (tail triangle)

best_path (value,_) [] = value
best_path (value,i) (first_row:triangle) = value + (max left_path right_path)
	where
		left_path = best_path (v,i) triangle
		right_path = best_path (w,j) triangle
		v = first_row !! i
		w = first_row !! j
		j = succ i
